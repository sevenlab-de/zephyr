...
{
	if (slot->state_now) {
		add_signal_phase(slot, DALI_TX_STOP_BIT_US, true);
		slot->index_max--;
	} else {
		add_signal_phase(slot, DALI_TX_STOP_BIT_US, false);
		slot->index_max--;
	}
}
	...
	case DALI_FRAME_CORRUPT:
		for (int_fast8_t i = 0; i < (2 * DALI_FRAME_BACKWARD_LENGTH); i++) {
			if (i == EXTEND_CORRUPT_PHASE) {
				...
				add_signal_phase(slot, DALI_TX_HALF_BIT_US, false);
			}
		}
		return;
...
static void rx_schedule_query(void)
{
	...
	const uint32_t query_count =
		counter_now + DALI_RX_FORWARD_BACK_MAX_US + GREY_AREA_INTERFRAME_US;
...
static void dali_tx_IRQHandler(const struct device *dev)
{
...
static void rx_process_stopbit_event(struct dali_lpc11u6x_data *data)
{
	...
	case BUS_FAILURE_DETECT:
		return;
	default:
		counter_rx_set_event_counter(STOPBIT, data->edge_count + DALI_FAILURE_CONDITION_US);
		counter_rx_event_enable(STOPBIT, true);
...
static int dali_lpc11u6x_init(const struct device *dev)
{
	...
	if (mcu_rx_pin()) {
		...
		data->rx_status = BUS_LOW;
		counter_rx_set_event_counter(STOPBIT, DALI_FAILURE_CONDITION_US);
		counter_rx_event_enable(STOPBIT, true);
	}
	return 0;
}
...
#define DALI_LPC11U6X_INIT(idx)                                                                    \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_IRQ_BY_IDX(DT_DRV_INST(idx), 0, irq),                               \
			    DT_IRQ_BY_IDX(DT_DRV_INST(idx), 0, priority), dali_tx_IRQHandler,      \
			    DEVICE_DT_INST_GET(idx), 0);                                           \
		irq_enable(DT_IRQ_BY_IDX(DT_DRV_INST(idx), 0, irq));                               \
		IRQ_CONNECT(DT_IRQ_BY_IDX(DT_DRV_INST(idx), 1, irq),                               \
			    DT_IRQ_BY_IDX(DT_DRV_INST(idx), 1, priority), dali_rx_IRQHandler,      \
			    DEVICE_DT_INST_GET(idx), 0);                                           \
