...
/* maximum is one signal per DALI bit plus a startbit and stop signal */
#define DALI_MAX_SIGNALS_PER_FRAME (DALI_MAX_BIT_PER_FRAME + 2)
...
struct dali_pwm_frame {
	 ...
	 */
	enum pwm_states signals[DALI_MAX_SIGNALS_PER_FRAME];
					    ...
					    uint32_t ticks, void *user_data)
{
	...
	__maybe_unused int ret = k_work_submit_to_queue(&dali_pwm_work_queue, &data->rx.work);

	__ASSERT(ret < 0, "submit to queue failed");
}
...
static void dali_pwm_process_stopbit_event(const struct device *dev)
{
	...
	case DALI_PWM_RX_DATA_BIT_START:
	case DALI_PWM_RX_DATA_BIT_INSIDE:
		LOG_DBG("{%08x:%02x %08x}", k_uptime_get_32(), data->rx.payload_length,
			data->rx.data);
				       ...
				       uint32_t ticks, void *user_data)
{
	...
	if (data->tx.pwm_frame.position >= DALI_MAX_SIGNALS_PER_FRAME) {
...
static int dali_pwm_generate_frame(const struct dali_frame *frame, struct dali_pwm_frame *pwm)
{
	...
	*pwm = (struct dali_pwm_frame){0};
	...
	/* add a finish */
	__ASSERT((signal_length < DALI_MAX_SIGNALS_PER_FRAME), "signal array overrun");
	pwm->signals[signal_length] = NONE;

	return 0;
}

static int dali_pwm_us_to_pwm_cycles(int us, uint64_t pwm_cycles_per_sec)
{
	return ((int64_t)pwm_cycles_per_sec * (int64_t)us) / USEC_PER_SEC;
}
